...
<?php
  ...
  $totalImageCountRes = $slider->geTotalImagesOfAStore($storeno);
  ...
  try {
    ...
    for ($i=0; $i <$total_image ; $i++) {
      ...
      $response[$i]=array();
        ...
        $target_dir = dirname(dirname(dirname(dirname(__FILE__))))."/assets/images/storemasterslider/";
        $target_file = $target_dir.$simagename;

        $insertImageRes = $slider->insertStoreSliderImages($storeno, $simagename);
