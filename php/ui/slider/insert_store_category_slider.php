...
<?php
  ...
  if(!isset($_FILES['storeimage']) || !isset($_POST['catno']) ){
      $response['error'] = true;
  ...
  $storepagedisplay = $store->getStoreDisplayNo($storeno, $catno);
  if($storepagedisplay->num_rows >0){
    $row = $storepagedisplay->fetch_array(MYSQLI_ASSOC);
 
    $displayno = $row['storepagedisplayno'];
  
    $totalImageCountRes = $slider->geTotalImagesOfACategory($displayno);
    ...
    try {
      ...
      for ($i=0; $i <$total_image ; $i++) {
        ...
        $file = $file_posted[$i];
        $response[$i]=array();
        // $displayno = (int)$storepagedisplayno;
          ...
          $insertImageRes = $slider->insertCategorySliderImages($displayno, $simagename);
    ...
    $response['error']= true;
    $response['message']= 'Display Category Not Found';
  }
